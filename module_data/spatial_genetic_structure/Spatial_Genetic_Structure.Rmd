...
[^2]: If you use the argument `genes=LocusName` to create a genind object for a single gene, you will need to manually edit the "occurrences.txt" file to contain just the coordinates of the individuals that have sequence data for that particular gene. Once you have created your genind object, you can use the command `indNames(seq_genind)` to obtain a list of accessions for all of the individuals in your genind object. Your "occurences.txt" file should contain these same individuals (and only these individuals), *in the same order*. 
